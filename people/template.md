#
# When someone leaves the lab:
#   1. Move their markdown file to the people/past/ folder.
#   2. Add a year_end field with the year they finished to the YAML header.
#   3. If they completed a PhD, add 'Dr.' to the start of their name.
#
---
# Fill out each field below. Do not remove the lines starting and ending with '---'.
# All fields are optional except 'name'.
# For any field you don't want to use, you can delete the line or leave it as is.

name: Full Name  # REQUIRED: Replace with your full name (e.g., Jane Doe)
vuwemail: firstname.lastname@vuw.ac.nz  # REQUIRED: Your VUW email address (used as a mailto link behind your name)
pronouns: they/them  # OPTIONAL: Your pronouns (e.g., she/her, he him, they/them)
photo: images/placeholder.png  # OPTIONAL: Path to your photo (or leave as is for a placeholder)
role: postdoc   # REQUIRED: Choose one - postdoc, ra, phd, or masters (e.g., ra)
year_started: 2026  # REQUIRED: The year you started (e.g., 2024)
year_end: 2026  # OPTIONAL: The year you finished (e.g., 2024)

links:
  # Add any links you want to display under your profile. Each link will show an icon if the key matches below.
  # To add a link, remove the '#' at the start of the line and replace the URL with your own.
  linkedin: https://linkedin.com/in/example        # LinkedIn icon
  funding: https://funding-link.com                # Money icon
  orcid: https://orcid.org/0000-0000-0000-0000     # ORCID icon
  scholar: https://scholar.google.com/citations?user=xxxx  # Google Scholar icon
  researchgate: https://www.researchgate.net/profile/Example # ResearchGate icon
  link1: https://example.com/first                 # Link icon (use for project, website, or any other link)
  link2: https://example.com/second                # Link icon
  link3: https://example.com/third                 # Link icon
---

# Write a short bio below. You can use **bold**, *italic*, and [links](https://example.com).
# Example: Jane is a PhD student studying marine ecology. She loves coding and diving.
# To reference the current projects page, use: [project name](../research.qmd)
Short bio goes here.
